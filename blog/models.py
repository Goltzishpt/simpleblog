...
class Profile(models.Model):
    ...
    profile_pic = ResizedImageField(size=[250, 250], blank=True, null=True,  crop=['top', 'left', 'middle', 'center'],
...
class Post(models.Model):
    ...
    header_image = ResizedImageField(size=[300, 300], crop=['top', 'left', 'middle', 'center'], blank=True, null=True,
                                     upload_to='images/')
...
class Comment(models.Model):
    ...
    def __str__(self):
        return f"{'%s - %s' % (self.post.title, self)} | {str(self.author)}"
